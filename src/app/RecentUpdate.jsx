...
const RecentUpdatePage = () => {
  ...
  useEffect(() => {
    (
      async () => {
        const dataApi = await getRecentUpdateComics(currentPage + 1)
        if (dataApi) {
  ...
  return (
    <main className='max-w-6xl mx-auto px-3'>
      <h2 className='text-white flex items-center gap-2 text-xl sm:text-2xl md:text-3xl font-bold mb-4 mt-4 md:mt-8'>
        <RxUpdate size={30} color='#10B981'/> Recently Update
      </h2>
      {data.length !== 0 &&
      <>
        <GridCard items={data}/>
...
export default RecentUpdatePage
